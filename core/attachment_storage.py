...
# External attachment links are intentionally much shorter lived than storage.
ATTACHMENT_URL_EXPIRATION_SECONDS = 300
...
def is_valid_attachment_id(file_id: str) -> bool:
    """Return whether an attachment ID is a canonical UUID generated by storage."""
    try:
...
def _external_attachment_url_configured() -> bool:
    return bool(os.getenv("WORKSPACE_EXTERNAL_URL", "").strip())
...
def validate_attachment_url_signature(
    file_id: str, expires: Optional[str], signature: Optional[str]
) -> bool:
    """Validate an external attachment URL's expiry and HMAC signature."""
    if not is_valid_attachment_id(file_id) or not expires or not signature:
        return False
...
def external_attachment_url_configured() -> bool:
    """Return whether attachment routes are exposed through an external URL."""
    return _external_attachment_url_configured()
...
def get_attachment_url(file_id: str) -> str:
    ...
    return f"{WORKSPACE_MCP_BASE_URI}:{WORKSPACE_MCP_PORT}/attachments/{file_id}"
