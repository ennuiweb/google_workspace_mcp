...
- Public MCP access remains `nginx -> mcp-auth-proxy -> backend`; temporary attachment downloads use a separate HTTPS nginx `/attachments/` route directly to the per-account loopback backend, protected by five-minute HMAC-signed bearer URLs (`WORKSPACE_EXTERNAL_URL` and distinct host-only `WORKSPACE_ATTACHMENT_SIGNING_SECRET` values per account). Google credentials remain in host-mounted per-account data directories.
